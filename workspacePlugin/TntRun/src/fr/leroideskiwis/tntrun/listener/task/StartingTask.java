...
public class StartingTask extends BukkitRunnable {
	...
	public void run() {
		...
		/*
		 * vérification du nombre de joueur : si il n'est pas égal au nombre maximum, alors cancel
		 * 
		 */
		
		if(main.getPlayers().size() < main.getMaxPlayers()){
			
			main.getPlayers().forEach(p -> p.sendMessage("§cPas assez de joueurs : compte a rebours arreté !"));
			cancel();
			return;
		}
		
		/*
		 * decrémentation de la variable rebours
		 */
		
		rebours--;
		
		/*
		 * si la variable rebours est à 0, lancer le jeu
		 * 
		 */
