		...
		if(main.senderIsPlayer(s)){
			...
			if(target == null){
				...
				if(target.equals(requester)){
					requester.sendMessage("§cVous ne pouvez pas vous envoyer de demande de téléportation a vous même !");
					return true;
				}
				
				main.tpRequests.add(new TpRequest(requester, target, Type.TPA));
				
				target.sendMessage("§c"+requester.getName()+" §7vous demande S'il peut se téléporter vers vous !");
				target.sendMessage("§7Faites §c/tpyes ou /tpaccept §7Pour accepter la demande de téléportation");
				target.sendMessage("§7Faites §c/tpdeny ou /tpno §7pour la refuser");
				
				requester.sendMessage("§7Votre demande de téléportation a bien été envoyée !");
